...
def objective(trial, args):
    ...
    ssm_d_state = trial.suggest_int("ssm_d_state", 64, 512, step=64)
    attn_headdim = trial.suggest_int("attn_headdim", 32, 64, step=32)
    mamba_layers = trial.suggest_int("mamba_layers", 2, 12)
    ...
    best_val_loss =trainer.fit(trial_dir, args.epochs, lr, **optim_kwargs)
...
def main(args):
    ...
    if not args.optuna:
        ...
        study.optimize(lambda trial: objective(trial, args), n_trials=50, timeout=57600)

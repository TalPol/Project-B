...
class Trainer:
        ...
        with progress_bar:
            ...
            for batch in self.train_loader:
                ...
                if loss_log is not None:
                    lr = lr_scheduler.get_last_lr() if lr_scheduler is not None else [pg["lr"] for pg in self.optimizer.param_groups]
                    if len(lr) == 1: lr = lr[0]
                    loss_log.append({
                        'chunks': chunks,
    ...
    def validate_one_epoch(self):
        ...
        with torch.no_grad():
            ...
            total_samples = sum(data.size(0) for data, _, _, *_ in self.valid_loader)
        ...
        return loss, np.mean(accs), np.median(accs), throughput
    ...
    def fit(self, workdir, epochs=1, lr=2e-3, **optim_kwargs):
        ...
        for epoch in range(1 + last_epoch, epochs + 1):
            try:
                ...
                val_loss, val_mean, val_median, throughput = self.validate_one_epoch()
            except KeyboardInterrupt:
                break

            print("[epoch {}] directory={} loss={:.4f} mean_acc={:.3f}% median_acc={:.3f}% throughput={:.3f} samples/s".format(
                epoch, workdir, val_loss, val_mean, val_median, throughput
            ))
    ...
    def fit_optuna(self, workdir, trial, epochs=1, lr=2e-3, **optim_kwargs):
        best_val_loss = float("inf")
        best_val_acc = 0.0
        if self.optimizer is None:
            self.init_optimizer(lr, **optim_kwargs)

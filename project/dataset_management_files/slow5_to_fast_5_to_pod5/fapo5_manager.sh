...
for input_folder in "$INPUT_MOTHER_FOLDER_PATH"; do
    ...
    if [ ! -d "$OUTPUT_MOTHER_FOLDER_PATH/$output_folder_name" ]; then
    ...
    if [ -n "$(find "$OUTPUT_MOTHER_FOLDER_PATH/$output_folder_name" -type f)" ]; then
        #Print the error massage
        echo "Folder: $OUTPUT_MOTHER_FOLDER_PATH/$output_folder_name contains files prior to the cuurent user's request regarding it."
    else
        ...
        pod5 convert fast5 "$input_folder"/*.fast5 --output $OUTPUT_MOTHER_FOLDER_PATH/$output_folder_name --one-to-one "$input_folder"
    fi
